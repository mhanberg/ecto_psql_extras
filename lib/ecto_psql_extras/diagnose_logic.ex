defmodule EctoPSQLExtras.DiagnoseLogic do
  ...
  @unused_indexes_min_size_bytes 1_000_000
  # 1 MB
  @null_indexes_min_size_mb 1
  # 50%
  @null_min_null_frac_percent 50
  @bloat_min_value 10.0
  # 33%
  @outliers_min_exec_ratio 33
        ...
        %{
          ...
          rows: [
            [
              false,
              "diagnose_error",
              "There was an error when generating your diagnose report, see logs"
            ]
          ]
        }
    end
  ...
  defp table_cache_hit(repo) do
    table_cache_hit_val =
      EctoPSQLExtras.cache_hit(repo, format: :raw).rows |> Enum.at(1) |> Enum.at(1)

    [ok, message] =
      case table_cache_hit_val do
        nil ->
  ...
  defp index_cache_hit(repo) do
    index_cache_hit_val =
      EctoPSQLExtras.cache_hit(repo, format: :raw).rows |> Enum.at(0) |> Enum.at(1)

    [ok, message] =
      case index_cache_hit_val do
        nil ->
  ...
  defp bloat(repo) do
    ...
    [ok, message] =
      case bloated_objects do
        [] ->
          [true, "No bloated tables or indexes detected."]

        _ ->
          print_bloat =
            Enum.map_join(bloated_objects, ", ", fn el ->
  ...
  defp duplicate_indexes(repo) do
    indexes =
      EctoPSQLExtras.duplicate_indexes(
        repo,
        format: :raw
      ).rows
    ...
    case EctoPSQLExtras.pg_stat_statements_version(repo) do
      nil ->
        [
          false,
          "outliers",
          "Cannot check outliers because 'pg_stat_statements' extension is not enabled."
        ]
  ...
  defp ssl_used(repo) do
    if EctoPSQLExtras.ssl_info_enabled(repo) do
      ...
      [
        false,
        "ssl_used",
        "Cannot check connection status because 'sslinfo' extension is not enabled."
      ]
    end
  end

  defp ssl_used_data(repo) do
    %Postgrex.Result{rows: [[ssl_used_result]]} =
      EctoPSQLExtras.ssl_used(
        repo,
        format: :raw
      )

    [ok, message] =
      case ssl_used_result do
        true ->
